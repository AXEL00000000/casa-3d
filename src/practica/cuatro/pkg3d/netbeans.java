...
public class Casa3D_SinEdicion implements GLEventListener,
    ...
    // =========================================================================
    //  RUTAS BASE - SE CALCULAN UNA SOLA VEZ AL INICIO
    // =========================================================================
    /**
     * Devuelve la ruta base del proyecto (donde está la carpeta src/).
     * NetBeans ejecuta con working dir = raíz del proyecto.
     */
    private static String projectRoot = detectProjectRoot();

    private static String detectProjectRoot() {
        // Working dir ya es la raíz del proyecto en NetBeans Run
        String wd = System.getProperty("user.dir", "");
        // Verificamos que exista src/ aquí
        if (new File(wd + "/src").exists()) return wd + "/";
        // Fallback: subir un nivel
        File parent = new File(wd).getParentFile();
        if (parent != null && new File(parent, "src").exists()) return parent.getAbsolutePath() + "/";
        return wd + "/";
    }
    ...
    private static class ObjModel {
        ...
        void load(String objPath) {
            ...
            File f = resolveFile(objPath);
            if (f == null) { System.err.println("OBJ no encontrado: " + objPath); return; }
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(f))) {
                ...
                loaded = true;
            } catch (Exception e) { System.err.println("OBJ error " + objPath + ": " + e.getMessage()); }
        }
        ...
        void loadMtl(File f) {
            if (!f.exists()) {
                // Intentar buscar el .mtl en la misma carpeta base
                System.err.println("MTL no encontrado: " + f.getAbsolutePath());
                return;
            }
            try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            } catch (Exception e) { System.err.println("MTL error: " + e.getMessage()); }
        }

        void initTextures(GL2 gl, String objPath) {
            // Carpetas donde buscar texturas (en orden de prioridad)
            String[] searchDirs = buildTextureDirs(baseDir, objPath);

            for (Map.Entry<String, Map<String, String>> me : materials.entrySet()) {
                String tn = me.getValue().get("map_Kd");
                if (tn == null) continue;
                tn = new File(tn).getName();
                me.getValue().put("map_Kd", tn);
                if (texCache.containsKey(tn)) continue;

                File tf = null;
                for (String dir : searchDirs) {
                    File candidate = new File(dir, tn);
                    if (candidate.exists()) { tf = candidate; break; }
                }
                ...
                if (tf != null && tf.exists()) {
                    try {
                        ...
                        texCache.put(tn, t);
                        System.out.println("Textura cargada: " + tf.getAbsolutePath());
                    } catch (Exception e) { System.err.println("Error tex " + tn + ": " + e.getMessage()); }
                } else {
                    System.err.println("Textura no encontrada: " + tn + " buscada en: " + Arrays.toString(searchDirs));
                }
            }
        }

        /**
         * Construye lista de directorios donde buscar texturas.
         * Cubre: junto al OBJ, subcarpeta Textures/textures, carpeta de texturas del piso.
         */
        static String[] buildTextureDirs(String baseDir, String objPath) {
            java.util.List<String> dirs = new ArrayList<>();
            // 1. Junto al archivo OBJ
            if (!baseDir.isEmpty()) dirs.add(baseDir);
            // 2. Subcarpeta Textures dentro del directorio del OBJ
            if (!baseDir.isEmpty()) {
                dirs.add(baseDir + "/Textures");
                dirs.add(baseDir + "/textures");
                dirs.add(baseDir + "/texture");
                dirs.add(baseDir + "/tex");
            }
            // 3. Rutas absolutas basadas en projectRoot
            dirs.add(projectRoot + "src/modelos/Models/OBJ format/Textures");
            dirs.add(projectRoot + "src/modelos/Models/OBJ format");
            dirs.add(projectRoot + "src/modelos/carrros/Car-Model");
            dirs.add(projectRoot + "src/textura");
            dirs.add(projectRoot + "textura");
            // 4. Versiones sin src/
            dirs.add(projectRoot + "modelos/Models/OBJ format/Textures");
            dirs.add(projectRoot + "modelos/Models/OBJ format");
        ...
        void render(GL2 gl, float[] colorOverride, boolean allowTexture) {
            ...
            for (Group g : groups) {
                ...
                if (colorOverride != null) gl.glColor3f(colorOverride[0], colorOverride[1], colorOverride[2]);
                    ...
                    String kd = mat.get("Kd");
                    if (kd != null) { String[] p = kd.split("\\s+"); if (p.length >= 3) gl.glColor3f(Float.parseFloat(p[0]), Float.parseFloat(p[1]), Float.parseFloat(p[2])); }
                ...
                for (Face face : g.faces) {
                    boolean ok = true;
                    for (int i = 0; i < 3; i++) if (face.vIdx[i] < 0 || face.vIdx[i] >= verts.size()) { ok = false; break; }
                    if (!ok) continue;
                    for (int i = 0; i < 3; i++) {
        ...
        void crearCajaGeom() {
            verts.add(new float[]{-0.5f,0,-0.5f}); verts.add(new float[]{ 0.5f,0,-0.5f});
            verts.add(new float[]{ 0.5f,1,-0.5f}); verts.add(new float[]{-0.5f,1,-0.5f});
            verts.add(new float[]{-0.5f,0, 0.5f}); verts.add(new float[]{ 0.5f,0, 0.5f});
            verts.add(new float[]{ 0.5f,1, 0.5f}); verts.add(new float[]{-0.5f,1, 0.5f});
            norms.add(new float[]{ 0, 0,-1}); norms.add(new float[]{ 0, 0, 1});
            norms.add(new float[]{-1, 0, 0}); norms.add(new float[]{ 1, 0, 0});
            norms.add(new float[]{ 0, 1, 0});
            texs.add(new float[]{0,0}); texs.add(new float[]{1,0}); texs.add(new float[]{1,1}); texs.add(new float[]{0,1});
            Group g = new Group();
        ...
        /**
         * Resuelve un path relativo al OBJ probando múltiples rutas base.
         * Cubre el caso de NetBeans donde src/ es el source root.
         */
        static File resolveFile(String path) {
            ...
            // 2. Con src/ prefijo (NetBeans: source packages están en src/)
            f = new File("src/" + path);
            if (f.exists()) return f;
            // 3. Desde projectRoot
            f = new File(projectRoot + path);
            if (f.exists()) return f;
            f = new File(projectRoot + "src/" + path);
            if (f.exists()) return f;
            // 4. Subir un nivel
            f = new File("../" + path);
            if (f.exists()) return f;
            System.err.println("No se pudo resolver: " + path);
            return null;
        }
    }
    ...
    private static class SceneObject {
        ...
        /** Objetos decorativos delgados/planos que no deben bloquear al jugador. */
        static boolean esObjSinColision(String path) {
            String n = new File(path).getName().toLowerCase(Locale.ROOT);
            // Solo objetos genuinamente planos/decorativos sin volumen físico relevante
            return n.contains("rug")         // alfombras (planas)
                || n.contains("television")  // TV colgada/delgada
                || n.contains("mirror")      // espejos (planos)
                || n.contains("doormat")     // felpudo (plano)
                // ELIMINADOS: plant, cardboard — sí deben tener colisión
                || n.equals("pared_plana");  // paredes → colisionParedesColocadas
        }
    ...
    private void crearObjetos() {
        ...
        // Carros — ruta directa a src/modelos/carrros/Car-Model/
        ...
        // Puertas — ruta directa a src/modelos/Models/OBJ format/
    ...
    private void add(String p, double px, double py, double pz,
                     double rx, double ry, double rz,
                     double sx, double sy, double sz) {
        SceneObject so = new SceneObject(p, px, py, pz, rx, ry, rz, sx, sy, sz);
        so.computeHitbox(); // calcular hitbox desde targetW/D ya disponibles
        sceneObjects.add(so);
    }

    /**
     * Convierte el path relativo usado en crearObjetos() al path real en disco.
     * NetBeans: los Source Packages están físicamente en src/ dentro del proyecto.
     */
    private static String resolveObjPath(String rel) {
        if ("pared_plana".equals(rel)) return "pared_plana";
        // Probar directamente (por si el working dir ya lo resuelve)
        if (new File(rel).exists()) return rel;
        // Probar con src/ (NetBeans)
        if (new File("src/" + rel).exists()) return "src/" + rel;
        // Probar con projectRoot
        if (new File(projectRoot + rel).exists()) return projectRoot + rel;
        if (new File(projectRoot + "src/" + rel).exists()) return projectRoot + "src/" + rel;
        // Fallback: devolver con src/ y dejar que resolveFile lo maneje
        return "src/" + rel;
    }

    private static boolean isCustomWall(String path) {
        return path.toLowerCase(Locale.ROOT).contains("pared_plana");
    }
    ...
    public Casa3D_SinEdicion() {
        System.out.println("Project root detectado: " + projectRoot);
        ...
        Thread t = new Thread(() -> {
            for (SceneObject so : sceneObjects) {
                so.model.load(resolveObjPath(so.objPath));
                so.computeScale();
            }
        }, "ObjLoader");
        t.setDaemon(true);
    ...
    private Texture cargarTex(GL2 gl, String nombre) {
        // Buscar en múltiples ubicaciones: src/textura/, textura/, y con projectRoot
        String[] rutas = {
            "src/textura/" + nombre,
            "textura/" + nombre,
            projectRoot + "src/textura/" + nombre,
            projectRoot + "textura/" + nombre,
            nombre
        };
        for (String ruta : rutas) {
            ...
            try {
                ...
                t.setTexParameteri(gl, GL2.GL_TEXTURE_MAG_FILTER, GL2.GL_LINEAR);
                System.out.println("Textura piso OK: " + ruta);
                return t;
            } catch (Exception e) { System.err.println("Tex " + nombre + ": " + e.getMessage()); }
        }
        System.err.println("Textura piso NO encontrada: " + nombre);
        return null;
    }
    ...
    private void dibujarHUD(GL2 gl, int w, int h) {
        ...
        if (fpsMode) {
            ...
            gl.glVertex2i(0,h); gl.glVertex2i(w,h); gl.glVertex2i(w,h-40); gl.glVertex2i(0,h-40);
            gl.glEnd();
            ...
            gl.glColor4f(0f,0f,0f,0.60f);
            gl.glBegin(GL2.GL_QUADS);
            gl.glVertex2i(0,36); gl.glVertex2i(w,36); gl.glVertex2i(w,0); gl.glVertex2i(0,0);
            gl.glEnd();
            gl.glDisable(GL2.GL_BLEND);
            if (textRenderer != null) {
                textRenderer.beginRendering(w, h);
    ...
    private void compilarDisplayLists(GL2 gl) {
        dlPisos = gl.glGenLists(1);
        gl.glNewList(dlPisos, GL2.GL_COMPILE); dibujarPisos(gl); gl.glEndList();
        dlParedesMerged = gl.glGenLists(1);
        gl.glNewList(dlParedesMerged, GL2.GL_COMPILE); dibujarParedesMerged(gl); gl.glEndList();
        dlCompiladas = true;
    }
    ...
    private void restoreColorMaterial(GL2 gl) { gl.glPopAttrib(); }
    ...
    private void dibujarParedesMerged(GL2 gl) {
        ...
        for (int y=0; y<=GH; y++) {
            ...
            for (int x=0; x<=GW; x++) {
                ...
                if (a && xS<0) xS=x;
                else if (!a && xS>=0) { dibujarSegmentoPared(gl, xS*CELL,0,y*CELL, x*CELL,0,y*CELL, WT,WH); xS=-1; }
            }
        }
        for (int x=0; x<=GW; x++) {
            int yS = -1;
            for (int y=0; y<=GH; y++) {
                ...
                if (a && yS<0) yS=y;
                else if (!a && yS>=0) { dibujarSegmentoPared(gl, x*CELL,0,yS*CELL, x*CELL,0,y*CELL, WT,WH); yS=-1; }
            }
        }
    ...
    private void quad(GL2 gl, double nx,double ny,double nz, double x0,double y0,double z0, double x1,double y1,double z1, double x2,double y2,double z2, double x3,double y3,double z3, double u,double v) {
        ...
        gl.glVertex3d(x0,y0,z0); gl.glVertex3d(x1,y1,z1); gl.glVertex3d(x2,y2,z2); gl.glVertex3d(x3,y3,z3);
        gl.glEnd();
    }
    ...
    private boolean hayColision(double x, double z) {
        ...
        if (colisionObjetos(x, z, radio)) return true;
        return false;
    }

    private boolean colisionParedesDelgadas(double x, double z, double r) {
        double hit = r + WT/2.0, hit2 = hit*hit;
        for (int y=0; y<=GH; y++) {
            int xS=-1;
            for (int xg=0; xg<=GW; xg++) {
                ...
                if (a && xS<0) xS=xg;
                    ...
                    if (distPointToSegmentSq(x,z, xS*CELL,y*CELL, xg*CELL,y*CELL) <= hit2) return true;
                    xS=-1;
                }
            }
        }
        for (int xg=0; xg<=GW; xg++) {
            ...
            for (int y=0; y<=GH; y++) {
                ...
                if (a && yS<0) yS=y;
                    ...
                    if (distPointToSegmentSq(x,z, xg*CELL,yS*CELL, xg*CELL,y*CELL) <= hit2) return true;
                    yS=-1;
                }
            }
        }
    ...
    /**
     * Colisión AABB contra todos los objetos de escena que tengan hitbox activo.
     * Excluye: wallMaterial (ya cubierto), puertas (tienen su propio sistema),
     *          objetos sin colisión (alfombras, TV, etc.), objetos solo al nivel del suelo
     *          cuya altura no alcanza al jugador (targetH < 0.25).
     */
    private boolean colisionObjetos(double x, double z, double r) {
        for (SceneObject so : sceneObjects) {
            if (so.wallMaterial) continue;
            if (so.noCollision)  continue;
            if (!so.model.loaded) continue;
            ...
            if (Math.abs(x - so.posX) < so.hitboxHalfX + r &&
                Math.abs(z - so.posZ) < so.hitboxHalfZ + r) {
                return true;
            }
        }
        return false;
    ...
    private static double distPointToSegmentSq(double px,double pz, double x1,double z1, double x2,double z2) {
        double vx=x2-x1, vz=z2-z1, wx=px-x1, wz=pz-z1;
        double c1=wx*vx+wz*vz; if(c1<=0) return wx*wx+wz*wz;
        double c2=vx*vx+vz*vz; if(c2<=c1){ double dx=px-x2,dz=pz-z2; return dx*dx+dz*dz; }
        double t=c1/c2, bx=x1+t*vx, bz=z1+t*vz, dx=px-bx, dz=pz-bz;
        return dx*dx+dz*dz;
    }
    ...
    private void salirFPS() { fpsMode=false; mouseCaptured=false; if(panel!=null) panel.setCursor(Cursor.getDefaultCursor()); }
    ...
    @Override public void mouseWheelMoved(MouseWheelEvent e) {
        if (!fpsMode) camDist = Math.max(5, Math.min(80, camDist+e.getWheelRotation()*1.5));
    }
